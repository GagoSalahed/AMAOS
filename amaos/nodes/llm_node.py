...
This module provides the LLM Node component responsible for model selection,
completions, and fallback support across multiple providers.
"""
...
# Base node protocol that all nodes will implement
class Node(Protocol):
    """Protocol defining the interface for all AMAOS nodes."""
...
class LLMNodeConfig(BaseModel):
    ...
    node_id: str = "llm_node"
    

from amaos.core.node_protocol import NodeTask, NodeResult

class LLMNode(Node):
    """Node responsible for model selection, completions, and fallback support.
    
    This node handles:
    - Selection of appropriate LLM models
    - Performing completions with proper formatting
    - Fallback across multiple providers
    - Tracking usage statistics and costs
    """

    def __init__(self, config: Optional[LLMNodeConfig] = None) -> None:
        """Initialize the LLM Node.
        
        Args:
            config: Configuration for the LLM Node.
        """
        # ✅ mapped from previous LLMManager
    ...
    async def handle(self, task: NodeTask) -> NodeResult:
        try:
            if task.task_type != "llm":
                return NodeResult(success=False, result="Unsupported task_type for LLMNode", error="unsupported_task_type", source=self.node_id)
            prompt = task.payload.get("prompt")
            if not prompt:
                return NodeResult(success=False, result="Missing 'prompt' in payload", error="missing_prompt", source=self.node_id)
            # Use the complete() method to get a response
            model_result = await self.complete(prompt)
            return NodeResult(
                success=(getattr(model_result, 'status', None) == ModelStatus.SUCCESS),
                result={"content": getattr(model_result, 'content', None)},
                latency=getattr(model_result, 'latency', None),
                source=self.node_id,
                error=getattr(model_result, 'error_message', None),
            )
        except Exception as e:
            return NodeResult(success=False, result="Exception in LLMNode", error=str(e), source=self.node_id)
